...
if ConfigData["LogPixelPaths"] != 0 and run % ConfigData["LogPixelPaths"] == 0:
    ...
    for id in range(0, NumElec, skip):
        ...
        plot(xpaths[id,0:maxsteps[id]], zxpaths[id,0:maxsteps[id]], color='blue', linewidth = 0.01)
        subplot(1,2,2,aspect=vertical_zoom)
        plot(ypaths[id,0:maxsteps[id]], zypaths[id,0:maxsteps[id]], color='blue', linewidth = 0.01)
...
if ConfigData["LogPixelPaths"] != 0 and run % ConfigData["LogPixelPaths"] == 0:
    ...
    for id in range(0, NumElec, skip):
        subplot(1,2,1,aspect=vertical_zoom)
        plot(xpaths[id,0:maxsteps[id]], zxpaths[id,0:maxsteps[id]], color='red', linewidth = 0.01)
        subplot(1,2,2,aspect=vertical_zoom)
        plot(ypaths[id,0:maxsteps[id]], zypaths[id,0:maxsteps[id]], color='red', linewidth = 0.01)
